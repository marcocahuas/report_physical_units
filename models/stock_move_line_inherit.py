...
class StockMoveLineInherit(models.Model):
    ...
    picking_type_use_existing_lots = fields.Boolean(related='picking_id.picking_type_id.use_existing_lots',
                                                    readonly=True)
        ...
        if 'product_uom_qty' in vals:
            for ml in self.filtered(
                ...
                if not ml.location_id.should_bypass_reservation():
                    qty_to_decrease = ml.product_qty - ml.product_uom_id._compute_quantity(vals['product_uom_qty'],
                ...
                if not updates.get('location_id', ml.location_id).should_bypass_reservation():
                    ...
                    try:
                        q = Quant._update_reserved_quantity(ml.product_id, updates.get('location_id', ml.location_id),
                                                            ml.product_qty, lot_id=updates.get('lot_id', ml.lot_id),
                    ...
                    if new_product_qty != ml.product_qty:
                        new_product_uom_qty = ml.product_id.uom_id._compute_quantity(new_product_qty, ml.product_uom_id,
                                                                                     rounding_method='HALF-UP')
    ...
    def unlink(self):
        ...
        for ml in self:
            ...
            if ml.product_id.type == 'product' and not ml.location_id.should_bypass_reservation() and not float_is_zero(
                ...
                try:
                    self.env['stock.quant']._update_reserved_quantity(ml.product_id, ml.location_id, -ml.product_qty,
                                                                      lot_id=ml.lot_id, package_id=ml.package_id,
                                                                      owner_id=ml.owner_id, strict=True)
                except UserError:
                    if ml.lot_id:
    ...
    def _log_message(self, record, move, template, vals):
        ...
        record.message_post_with_view(template, values={'move': move, 'vals': dict(vals, **data)},
